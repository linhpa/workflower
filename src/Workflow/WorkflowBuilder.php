...
class WorkflowBuilder
{
     ...
     * @param string     $name
     */
    public function addInclusiveGateway($id, string $participant, string $name = null): void
    {
        $this->inclusiveGateways[$id] = [$participant, $name];
    }
    ...
    public function build()
    {
        ...
        foreach ($this->inclusiveGateways as $id => $gateway) {
            list($roleId, $name) = $gateway;
            $this->assertWorkflowHasRole($workflow, $roleId);

            $workflow->addFlowObject(new InclusiveGateway($id, $workflow->getRole($roleId), $name));
        }
